...
def update_brigada_id(db: Session, form_data: schemas.UpdateBrigadaSch):
    ...
    if db_update_brigada:
        ...
        if form_data.topic_id is not None:
            db_update_brigada.topic_id = form_data.topic_id
...
def acceptreject(db: Session, form_data: schemas.AcceptRejectRequest, user):
    ...
    if db_get:
        ...
        if form_data.status in [3,4,6,8]:
...
def filter_requests_all(
    ...
    query = db.query(models.Requests).join(models.Category).join(models.Requests.user)
