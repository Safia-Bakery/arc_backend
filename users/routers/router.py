...
@user_router.get(
    "/all/permissions",
    summary="from this api you can get list of roles",
    response_model=list[schema.ParentPage],
)
...
async def user_group(
    group_data: schema.CreateGroupSch,
    db: Session = Depends(get_db),
    request_user: schema.UserFullBack = Depends(get_current_user),
):
...
async def user_group_update(
    group_data: schema.UpdateGroupSch,
    db: Session = Depends(get_db),
    request_user: schema.UserFullBack = Depends(get_current_user),
):
